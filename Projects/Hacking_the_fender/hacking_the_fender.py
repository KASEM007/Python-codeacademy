...
# You must acquire access to The Fender‘s systems, you must update his "passwords.txt"
# file to scramble the secret data. The last thing you need to do is add the
# signature of Slash Null, a different hacker whose nefarious deeds could be very
...
# retrieve, manipulate, obscure, and create data in your quest for justice.
# Work with CSV files and other text files in this exploration of the strength of
# Python file programming.
...
3.
Next we’ll need you to open up the file itself. 
Store it in a file object called password_file.
...
7.
Remove the print statement. We want to add each username to the list of compromised_users. 
Use the list’s .append() method to add the username to 
...
11.
Exit out of that with block. You’re doing great so far! 
We’ve got the data we need to employ as insurance against The Fender.
...
Create a new with block and open "new_passwords.csv" 
in write-mode. Save the file object to a variable called new_passwords_obj.


17.
Enemy of the people, Slash Null, is who we want The Fender to think was behind this attack. 
He has a signature, whenever he hacks someone he adds this signature to one of the files 
he touches. Here is the signature:
...
19.
What an incredible success! We’ll take care of moving the new passwords file over the old 
one in case you want to practice hacking The Fender in the future.
