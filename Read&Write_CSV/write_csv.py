...
# Naturally if we have the ability to read different CSV files we might want to be able to
# programmatically create CSV files that save output and data that someone could load into
# their spreadsheet software. Let’s say we have a big list of data that we want to save into
# a CSV file. We could do the following:
...
# CSV file in write-mode by passing the 'w' argument to the open() function.
...
# First we want the headers, so we call .writeheader() on the writer object. This writes all the
# fields passed to fieldnames as the first row in our file. Then we iterate through our big_list
# of data. Each item in big_list is a dictionary with each field in fields as the keys.
# We call output_writer.writerow() with the item dictionaries which writes each line to
# the CSV file.
...
# 1.
# We have a list in the access_log which is a list of dictionaries
...
# Don’t forget to open the file in write-mode.

# 3.
# Create a csv.DictWriter instance called log_writer.
# Pass logger_csv as the first argument and then fields as
# a keyword argument to the keyword fieldnames.
